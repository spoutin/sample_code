...
def get_mongo_client(
    mongoServers: str, mongoReplicaset: str, username: str, password: str
):
...
def init_aludata_leak_reporting_table(client):
    ...
    create_mysql_table(
        client, reportingTableCreateQuery, REPORTING_AULDATALEAK_TABLENAME
    )
    create_mysql_table(
        client, reportingTableCreateIndex, REPORTING_AULDATALEAK_TABLENAME
    )
    ...
    if len(subList) > 0:
        ...
        if len(usageResult) > 0:
            ...
            for index, row in usageResult.iterrows():
                usageResultReportingQuery = (
                    usageResultReportingQuery
                    + f"('{row['extSubId']}', {row['MDN']}, {row['BAN']}, '{row['start']}', '{row['end']}', '{int(row['bytesIn']) + int(row['bytesOut'])}', '{auditDate}'),"
                )
            usageResultReportingQuery = usageResultReportingQuery[:-1]
            reportingClient.execute(usageResultReportingQuery)
            print(
                usageResult.size + " rows written to " + REPORTING_AULDATALEAK_TABLENAME
            )
